    ...
    ({ origin, data }: MessageEvent<SetCertificationResults<TApiIdentifier> | SetModuleResults>) => {
      ...
      if (origin.startsWith("vscode-webview://") || origin === "null") {
        ...
        if (command === "setCertificationResults") {
          ...
          // Check if this is a "no APIs found" error vs a network error
          const extensionError = new Error();
          // Si no hay resultados o todos son errores, no setear error
          if (
            !certification ||
            ...
            setError(null); // No mostrar error, dejar que el componente muestre "No APIs found"
          } else {
