...
import { Flex, ScrollArea, Tabs, Badge } from "@mantine/core";
...
export default function ApiTabs<TApiIdentifier extends ApiIdentifier = ApiIdentifier>({
  ...
  return (
    ...
    >
      <Tabs.List>
        {apis.map((api) => {
          const { apiName, score, apiType } = api;
          const id = getApiIdentifier(api);
          
          // Debug log para ver qué valores llegan
          console.log('API Debug:', { apiName, apiType, hasApiType: !!apiType, isWSO2: apiType === 'WSO2', isMETADATA: apiType === 'METADATA' });
